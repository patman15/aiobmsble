...
class MockBuknuwoBleakClient(MockBleakClient):
    ...
    _RESP: dict[bytes, bytes] = {
        b"\x01\x03\x00\x0b\x00\x01\xf5\xc8": b"\x01\x03\x02\x0c\x00\xbd\x44",  #  info
        b"\x01\x03\x00\x2f\x00\x0a\xf4\x04": (
            b"\x01\x03\x14\x00\xc3\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
            b"\x00\x00\xdb\x32"
        ),  #  cell temperatures
        b"\x01\x03\x00\x2e\x00\x01\xe4\x03": (b"\x01\x03\x02\x00\x01\x79\x84"),  #  info
        b"\x01\x03\x00\x39\x00\x01\x54\x07": b"\x01\x03\x02\x00\xbb\xf8\x37",  #  MOS FET temperature
        b"\x01\x03\x00\x09\x00\x03\xd5\xc9": b"\x01\x03\x06\x00\x00\x00\x00\x0c\x00\x24\x75",  #  info
        b"\x01\x03\x00\x02\x00\x01\x25\xca": (b"\x01\x03\x02\x00\x61\x79\xac"),  #  SoC
        b"\x01\x03\x00\x01\x00\x01\xd5\xca": b"\x01\x03\x02\x05\x46\x3a\xe6",  #  voltage
        b"\x01\x03\x00\x00\x00\x01\x84\x0a": b"\x01\x03\x02\xde\x78\xe1\xc6",  #  current
        b"\x01\x03\x00\xa2\x00\x01\x25\xe8": b"\x01\x03\x02\x23\x28\xa1\x6a",
        b"\x01\x03\x00\x00\x00\x0d\x84\x0f": (
        ...
        b"\x01\x03\x00\x2e\x00\x0b\x64\x04": (
    ...
    async def write_gatt_char(
        ...
        _response: Final[bytearray] = bytearray(self._RESP.get(bytes(data), b""))
        for notify_data in [
...
async def test_update(patch_bleak_client, keep_alive_fixture: bool) -> None:
    ...
    assert await bms.async_update() == {
        ...
        "design_capacity": 90,
        "temp_sensors": 1,
        "temp_values": [TS(18.7, TS.T.MOSFET), TS(19.5, TS.T.CELL)],
        "power": 53.117,
        "problem": True,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        b"\x01\x03",
        (
            b"\x01\x04\x1a\x01\x84\x05\x59\x00\x63\x00\x64\x20\x56\x20\xa1\x23\x28\x00\x01\x00\x00"
            b"\x00\x04\x80\x00\x2d\x00\x00\x00\x2d\xdb"
        ),
        (
            b"\x01\x03\x1a\x01\x84\x05\x59\x00\x63\x00\x64\x20\x56\x20\xa1\x23\x28\x00\x01\x00\x00"
            b"\x00\x04\x80\x00\x2d\x00\x00\x00\xff\xff"
        ),
