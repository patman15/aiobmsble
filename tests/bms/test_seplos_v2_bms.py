...
REF_VALUE: BMSSample = {
    ...
    "temp_values": [TS(v, TS.T.CELL) for v in (23.2, 23.1, 22.9, 23.4)]
    + [TS(29.3, TS.T.AMBIENT), TS(24.5, TS.T.MOSFET)],
    "delta_voltage": 0.022,
    "pack_count": 1,
    "balancer": 2,
    "chrg_mosfet": True,
...
class MockSeplosv2BleakClient(MockBleakClient):
    ...
    RESP: dict[bytes, bytes] = {
        b"\x00\x46\x61\x00\x01\x00\xf7\xc1": (  # get single machine data, address is incorrect but device also ignores it
            # 54.43V, 11.8A, 298.53Ah, 98.2%, 214 cycles, 306Ah, 16 cells, 6 temp sensors
            b"\x7e\x14\x00\x61\x00\x00\x6a\x00\x00\x10\x0d\x44\x0d\x47\x0d\x4c\x0d\x47\x0d\x45\x0d"
            b"\x46\x0d\x46\x0d\x58\x0d\x58\x0d\x59\x0d\x48\x0d\x44\x0d\x48\x0d\x43\x0d\x48\x0d\x47"
            b"\x06\x0b\x93\x0b\x92\x0b\x90\x0b\x95\x0b\xd0\x0b\xa0\x04\x9c\x15\x43\x74\x9d\x06\x76"
            b"\xc0\x03\xd6\x76\xc0\x00\xd6\x03\xe8\x15\x46\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
            b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x03\x08\x00\x00\x00\x00"
            b"\x00\x00\x00\x00\x00\x02\x00\x00\xd8\xc3\x0d"
        ),
        b"\00\x46\x51\x00\x00\x3a\x7f": (  # manufacturer information
            b"\x7e\x14\x00\x51\x00\x00\x24\x43\x41\x4e\x3a\x50\x4e\x47\x5f\x44\x59\x45\x5f\x4c\x75"
            b"\x78\x70\x5f\x54\x42\x42\x31\x31\x30\x31\x2d\x53\x50\x37\x36\x20\x10\x06\x01\x01\x46"
            b"\x01\xa2\x6c\x0d"
        ),
        b"\x00\x46\x62\x00\x00\xa6\x8a": (  # GPD for main pack
            b"\x7e\x14\x00\x62\x00\x00\x30\x00\x00\x10\x0c\xf4\x0c\xee\x06\x0b\x93\x0b\x7f\x0b\xb6"
            b"\x0b\x8d\x00\xd7\x14\xb4\x11\x14\x07\x20\xd0\x02\x08\x20\xd0\x00\x71\x03\xe8\x14\xb9"
            b"\x07\x00\x02\x03\x08\x00\x00\x00\x00\x00\x00\x00\x00\x76\x31\x0d"
        ),
    ...
    def _response(
        ...
        if (
            ...
            if (
                ...
                return bytearray(self.RESP.get(bytes(data)[2:-1], b""))
...
async def test_short_message(
    ...
    monkeypatch.setattr(
        ...
        | {
            b"\x00\x46\x61\x00\x01\x00\xf7\xc1": (
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            + b"\xff\xff\xff\xff\xff\xff\xff\xff\x00\x02\x00\x00\x82\x2d\x0d",
            0xFFFFFFFFFF7F003F,

        ),
        (
            MockSeplosv2BleakClient.RESP[b"\x00\x46\x61\x00\x01\x00\xf7\xc1"][:101]
            + b"\x00\x00\x02\x00\x00\x00\x00\x00\x00\x02\x00\x00\x06\x49\x0d",
            0x20000000000,  # Charging over-temp protection
        ),
        (
            MockSeplosv2BleakClient.RESP[b"\x00\x46\x61\x00\x01\x00\xf7\xc1"][:101]
...
async def test_problem_response(
    ...
    monkeypatch.setattr(
        MockSeplosv2BleakClient,
        "RESP",
        MockSeplosv2BleakClient.RESP | {b"\x00\x46\x61\x00\x01\x00\xf7\xc1": problem},
    )
